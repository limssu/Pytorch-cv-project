...
본 연구는 대규모 반도체 웨이퍼 데이터셋의 안정적인 학습 및 고속 추론 성능을 보장하기 위해, 고성능 클라우드 인프라 기반의 End-to-End 딥러닝 파이프라인을 가동하였다.
...
지도 교수 피드백 및 학사 일정에 맞춘 반도체 웨이퍼 결함 탐지 프로젝트의 최종 발표 마일스톤이다.
...
- [x] **06.02** : **AWS 가상 머신 이주 및 교수님 대면 중간 점검 피드백 반영** 🔍
  - *수행 항목*: AWS EC2 고성능 GPU 인프라 환경 구축 완료 및 AutoEncoder 초기 모델 스코어 중간 점검 완수
  - **👍 지도 교수 주요 피드백 및 수정 요구사항 수용**:
...
- [x] **06.09** : **보강 주간 및 피드백 기반 3종 경기 모델 총공세 (수업 없음)** 💻
...
현업 인라인(In-line) 자동 검사 시스템(ADC) 도입 가능성을 타당성 있게 검증하기 위해 학술적·실무적 관점에서 다음과 같은 명확한 평가 지표를 적용하여 최종 모델들을 채점하였다.
...
반도체 양산 공정은 수율이 매우 높아 불량 데이터의 절대적 빈도가 낮은 **데이터 불균형(Data Imbalance)** 문제가 발생하며, 신종 불량의 형태를 예측하기 어렵다. 본 연구는 이 한계를 극복하기 위해 정상(Normal) 데이터만을 활용해 특징 공간을 학습하는 **비지도 학습(Unsupervised Learning)** 기법을 확립하였다.
...
교수님 피드백을 반영하여 **알고리즘 패러다임 자체를 전환하는 3대 비전 모델 교차 벤치마크 실험**을 완수하였다. 

동일한 대규모 학습 데이터셋(**정상 115,544장**)으로 각 엔진을 훈련한 후, 9종의 결함 유형과 정상 데이터가 고스란히 혼합된 **실전 파이널 시험지 3,600장**을 대상으로 최종 분류 정확도($\text{Accuracy}$)를 도출한 결과는 다음과 같다.
...
프로젝트 인프라 디렉토리를 다음과 같이 정석 구조로 정돈 및 구조화하였다.

```text
├── anomaly/
│   ├── colab/                   # Google Colab 선행 연구 아카이브 폴더
│   │   └── Pytorch_cv_project.ipynb  # 초기 프로토타이핑 및 데이터 파이프라인 검증 주피터 노트북
